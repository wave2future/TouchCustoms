...
NSString *const SCAsyncSocketException = @"SCAsyncSocketException";
NSString *const SCAsyncSocketErrorDomain = @"SCAsyncSocketErrorDomain";
...
@interface SCAsyncSocket (Private)
 ...
 **/
@interface SCAsyncReadPacket : NSObject {
...
@implementation SCAsyncReadPacket
...
- (unsigned)readLengthForTerm
{
	assert(term && @"Searching for term in data when there is no term.");
 ...
 **/
@interface SCAsyncWritePacket : NSObject {
...
@implementation SCAsyncWritePacket
...
/**
 * The SCAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
 **/
@interface SCAsyncSpecialPacket : NSObject
{
@public
...
@implementation SCAsyncSpecialPacket
...
{
	if (theDelegate == NULL)
		[NSException raise:SCAsyncSocketException format:@"Attempting to accept without a delegate. Set a delegate first."];
	
	if (theSocket != NULL || theSocket6 != NULL)
		[NSException raise:SCAsyncSocketException format:@"Attempting to accept while connected or accepting connections. Disconnect first."];
				...
				error:(NSError **)errPtr
{
	if(theDelegate == NULL)
	{
		...
		[NSException raise:SCAsyncSocketException format:message];
	}
...
- (void)doAcceptWithSocket:(CFSocketNativeHandle)newNative
{
	...
	SCAsyncSocket *newSocket = [[[[self class] alloc] initWithDelegate:theDelegate] autorelease];
...
- (NSError *)getSocketError
{
	...
	return [NSError errorWithDomain:SCAsyncSocketErrorDomain code:SCAsyncSocketCFSocketError userInfo:info];
}
...
- (NSError *)getAbortError
{
	...
	return [NSError errorWithDomain:SCAsyncSocketErrorDomain code:SCAsyncSocketCanceledError userInfo:info];
}
...
- (NSError *)getConnectTimeoutError
{
	...
	return [NSError errorWithDomain:SCAsyncSocketErrorDomain code:SCAsyncSocketConnectTimeoutError userInfo:info];
}
...
- (NSError *)getReadMaxedOutError
{
	...
	return [NSError errorWithDomain:SCAsyncSocketErrorDomain code:SCAsyncSocketReadMaxedOutError userInfo:info];
}
...
- (NSError *)getReadTimeoutError
{
	...
	return [NSError errorWithDomain:SCAsyncSocketErrorDomain code:SCAsyncSocketReadTimeoutError userInfo:info];
}
...
- (NSError *)getWriteTimeoutError
{
	...
	return [NSError errorWithDomain:SCAsyncSocketErrorDomain code:SCAsyncSocketWriteTimeoutError userInfo:info];
}
...
- (void)readDataToLength:(CFIndex)length withTimeout:(NSTimeInterval)timeout tag:(long)tag;
{
	...
	SCAsyncReadPacket *packet = [[SCAsyncReadPacket alloc] initWithData:buffer
															timeout:timeout
																tag:tag
												   readAllAvailable:NO
														 terminator:nil
...
- (void)maybeDequeueRead
{
	...
	if((theCurrentRead == nil) && (theReadStream != NULL))
	{
		if([theReadQueue count] > 0)
		{
			...
			if([theCurrentRead isKindOfClass:[SCAsyncSpecialPacket class]])
			{
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag;
{
	...
	SCAsyncWritePacket *packet = [[SCAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)maybeDequeueWrite
{
	...
	if((theCurrentWrite == nil) && (theWriteStream != NULL))
	{
		if([theWriteQueue count] > 0)
		{
			...
			if([theCurrentWrite isKindOfClass:[SCAsyncSpecialPacket class]])
			{
				// Attempt to start TLS
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	SCAsyncSpecialPacket *packet = [[SCAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	...
	SCAsyncSocket *socket = [[(SCAsyncSocket *)pInfo retain] autorelease];
