...
@class SCAsyncSocket;
@class SCAsyncReadPacket;
@class SCAsyncWritePacket;

extern NSString *const SCAsyncSocketException;
extern NSString *const SCAsyncSocketErrorDomain;

enum SCAsyncSocketError {
	SCAsyncSocketCFSocketError = kCFSocketError,		// From CFSocketError enum.
	SCAsyncSocketNoError = 0,							// Never used.
	SCAsyncSocketCanceledError,							// onSocketWillConnect: returned NO.
	SCAsyncSocketConnectTimeoutError,
	SCAsyncSocketReadMaxedOutError,						// Reached set maxLength without completing
	SCAsyncSocketReadTimeoutError,
	SCAsyncSocketWriteTimeoutError
};

@interface NSObject (SCAsyncSocketDelegate)
...
**/
- (void)onSocket:(SCAsyncSocket *)sock willDisconnectWithError:(NSError *)err;

/**
 * Called when a socket disconnects with or without error.  If you want to release a socket after it disconnects,
 * do so here. It is not safe to do that during "onSocket:willDisconnectWithError:".
**/
- (void)onSocketDidDisconnect:(SCAsyncSocket *)sock;

/**
 * Called when a socket accepts a connection.  Another socket is spawned to handle it. The new socket will have
 * the same delegate and will call "onSocket:didConnectToHost:port:".
**/
- (void)onSocket:(SCAsyncSocket *)sock didAcceptNewSocket:(SCAsyncSocket *)newSocket;

/**
 * Called when a new socket is spawned to handle a connection.  This method should return the run-loop of the
 * thread on which the new socket and its delegate should operate. If omitted, [NSRunLoop currentRunLoop] is used.
**/
- (NSRunLoop *)onSocket:(SCAsyncSocket *)sock wantsRunLoopForNewSocket:(SCAsyncSocket *)newSocket;
...
**/
- (BOOL)onSocketWillConnect:(SCAsyncSocket *)sock;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)onSocket:(SCAsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)onSocket:(SCAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)onSocket:(SCAsyncSocket *)sock didReadPartialDataOfLength:(CFIndex)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)onSocket:(SCAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called after the socket has completed SSL/TLS negotiation.
 * This method is not called unless you use the provided startTLS method.
**/
- (void)onSocket:(SCAsyncSocket *)sock didSecure:(BOOL)flag;
...
@interface SCAsyncSocket : NSObject {
	...
	SCAsyncReadPacket *theCurrentRead;
	NSTimer *theReadTimer;
	NSMutableData *partialReadBuffer;
	
	NSMutableArray *theWriteQueue;
	SCAsyncWritePacket *theCurrentWrite;
...
// Once one of the accept or connect methods are called, the SCAsyncSocket instance is locked in
...
// When an incoming connection is accepted, SCAsyncSocket invokes several delegate methods.
 ...
 * When a connection comes in, the SCAsyncSocket instance will call the various delegate methods (see above).
