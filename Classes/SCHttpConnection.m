...
@interface SCHttpConnection (PrivateAPI)
...
- (BOOL)isAuthenticated
{
	...
	SCHttpAuthenticationRequest *auth = [[[SCHttpAuthenticationRequest alloc] initWithRequest:request] autorelease];
...
- (BOOL)parseRangeRequest:(NSString *)rangeHeader withContentLength:(UInt64)contentLength
{
	...
	for(i = 0; i < [rangeComponents count]; i++)
	{
		...
		if(dashRange.location == NSNotFound)
		{
			...
			[ranges addObject:[NSValue valueWithSCRange:SCMakeRange(byteIndex, 1)]];
		}
		else
		{
			...
			if(!hasR1)
			{
				...
				[ranges addObject:[NSValue valueWithSCRange:SCMakeRange(startIndex, r2)]];
			}
			else if(!hasR2)
			{
				// We're dealing with a "[#]-" range
				...
				[ranges addObject:[NSValue valueWithSCRange:SCMakeRange(r1, contentLength - r1)]];
			}
			else
			{
				...
				[ranges addObject:[NSValue valueWithSCRange:SCMakeRange(r1, r2 - r1 + 1)]];
			}
		}
	}
	...
	{
		SCRange range1 = [[ranges objectAtIndex:i] SCrangeValue];
		
		NSUInteger j;
		for(j = i+1; j < [ranges count]; j++)
		{
			SCRange range2 = [[ranges objectAtIndex:j] SCrangeValue];
			
			SCRange iRange = SCIntersectionRange(range1, range2);
	...
	else
	{
		...
		else
		{
			...
			if([ranges count] == 1)
			{
				...
				SCRange range = [[ranges objectAtIndex:0] SCrangeValue];
...
- (CFHTTPMessageRef)prepareUniRangeResponse:(UInt64)contentLength
{
	...
	NSString *rangeStr = [NSString stringWithFormat:@"%qu-%qu", range.location, SCMaxRange(range) - 1];
...
- (CFHTTPMessageRef)prepareMultiRangeResponse:(UInt64)contentLength
{
	...
	{
		SCRange range = [[ranges objectAtIndex:i] SCrangeValue];
...
- (NSObject<SCHttpResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
	...
	{
		return [[[SCHttpFileResponse alloc] initWithFilePath:filePath] autorelease];
	}
...
- (void)onSocket:(SCAsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port
{
	...
	[asyncSocket readDataToData:[SCAsyncSocket CRLFData]
...
- (void)onSocket:(SCAsyncSocket *)sock didWriteDataWithTag:(long)tag
{
	...
	else if(tag == HTTP_PARTIAL_RANGES_RESPONSE_BODY)
	{
		...
		SCRange range = [[ranges objectAtIndex:rangeIndex] SCrangeValue];
		...
		else
		{
			if(++rangeIndex < [ranges count])
			{
				...
				// Start writing range body
				range = [[ranges objectAtIndex:rangeIndex] SCrangeValue];
